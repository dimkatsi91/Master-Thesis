...
int main ( int argc, char **argv )
{
	...
	// Save Images For later process :
	imwrite("ORIGINAL_IMAGE.png", image);
	imwrite("CUTTED_QR.png", qr_thres);
		
	/* Remove 10x10 border from qr_thres image ::  */
	int QR_IM[100][100];
	Mat QR_QR;
	QR_QR = Mat::zeros(100, 100, CV_8UC1 );
	int qrows=0, qcols=0;
	int i,j;
	
	for(i=10;i<110;i++) {
	  for(j=10;j<110;j++) {
	    QR_IM[i-10][j-10] = (int) *(qr_thres.data + j + 120*i);
	  }
	}
	
	for(i=0;i<100;i++) {
	  for(j=0;j<100;j++) {
	    *(QR_QR.data + j + 100*i) = QR_IM[i][j];
	  }
	}
      
	//imshow("QR_QR", QR_QR);
	//resizeWindow("QR_QR", 200, 200);
	
	resize(QR_QR, QR_QR, Size(21,21), 0, 0, INTER_CUBIC);
	//imshow("QR_LAST", QR_QR);
	//resizeWindow("QR_LAST", 200, 200);
	//waitKey(0);
	
	int myQR[21][21];
	int width = QR_QR.cols;
	int height = QR_QR.rows;
	for(i=0;i<height;i++) {
	  for(j=0;j<width;j++) {
	    myQR[i][j] = (int) *(QR_QR.data + j + width*i); 
	    myQR[i][j] = (!myQR[i][j] ? 1 : 0);
	    //cout << (!QR_IM[i][j] ? "O " : "X " );
	    //cout << myQR[i][j] << " " ;
	    //cout << " ";
	  }
