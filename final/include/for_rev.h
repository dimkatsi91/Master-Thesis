 ...
 * Created on November 7, 2016, 12:10 PM
 *
 * Forward and reverse matrices
 * Forward : Exponent of a   ->  Integer number
 * Reverse : Integer number  ->  Exponent of a
 */
#ifndef FOR_REV_H
#define FOR_REV_H
...
int rs_forward[GF_LEN] = {1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,152,45,90,180,117,234,201,143,3,6,12,24,48,96,192,
...
int rs_reverse[GF_LEN] = {0,0,1,25,2,50,26,198,3,223,51,238,27,104,199,75,4,100,224,14,52,141,239,129,28,193,105,248,200,8,76,113,
...
class GF_Exception : public exception {
public:
	GF_Exception(int n) : number(n) {};
	int number;
	const char* what() {
		return " > 255. Error!\n";
	}
...
int ret_integer(int a_power)
{
    ...
    try {
		if(a_power>255)
			throw GF_Exception(a_power);
		else
			return rs_forward[a_power];
	}
	catch(GF_Exception &e) {
		cout << "Alpha's Power" << e.what();
		exit(1);
	}
}
...
int ret_a_power(int akeraios)
{
    ...
    try {
		if(akeraios>255)
			throw GF_Exception(akeraios);
		else
			return rs_reverse[akeraios];
	}
	catch(GF_Exception &e) {
		cout << "Integer" << e.what();
		exit(1);
